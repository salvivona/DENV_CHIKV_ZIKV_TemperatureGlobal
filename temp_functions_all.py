...
def quad_trunc(T,c,Tm,T0):
  qd=-c
  b = 1*((qd*(T-T0)*(T-Tm))>1) + (qd*(T-T0)*(T-Tm)*(T<=Tm)*(T>=T0))*((qd*(T-T0)*(T-Tm))<1)
  return b
...
def briere(t,c,Tm, T0): 
   
  b=np.zeros(len(t))
   
  for i in range(0,len(t)-1): 
    if t[i]>T0 and t[i]<Tm:
      b[i] = c*t[i]*(t[i]-T0)*np.sqrt(Tm-t[i])
    else: 
      b[i]= 0 
  return b
...
'''
def quad_2(T,c,Tm,T0):
  qd=-c
  b = qd*(T-T0)*(T-Tm)*(T<=Tm)*(T>=T0)
  return b
'''
